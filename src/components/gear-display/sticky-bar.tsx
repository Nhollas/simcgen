...
export function StickyBar() {
  ...
  const [searchedItems, searchOpen] = form.watch([
    "searchedItems",
    "searchOpen",
  ])

  return (
    <div className="sticky inset-x-0 top-6 z-50 flex justify-center">
      <div className="grid w-full max-w-lg grid-cols-2 gap-4 rounded-lg bg-muted/75 p-2">
        <Popover
          open={searchOpen}
          onOpenChange={form.setValue.bind(form, "searchOpen")}
        >
          <PopoverTrigger
...
function handleSelectItem(item: string, form: UseFormReturn<GearOutputSchema>) {
  ...
  const selectedItem = searchedItems.find((item) => item.unique_id === itemId)

  if (!selectedItem) {
    return
  }

  const slot = inventoryTypeToSlot(selectedItem.inventoryType)
  ...
  form.setValue(`gearInfo.${slot}`, [...(existingItems || []), selectedItem])
